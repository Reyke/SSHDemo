...
@Entity
@Table(name = "issues")
@DiscriminatorValue(Constants.ISSUE_TYPE_POSSIBLEDEFECT)
public class PossibleDefect extends CommonIssue {
	
	public PossibleDefect() {
	}

	public PossibleDefect(Issues issues) {
		super(issues);
	}
	...
	public boolean isValidate() {
		if (isEmpty(title) || isEmpty(errorPageName) || isEmpty(actualResult)
				|| isEmpty(expectedResult) || isEmpty(reproducedSteps)) {
			return false;
		} else {
			return true;
		}
